...
class SwingStrategy:
    ...
    def __init__(self, config=None):
        ...
        self.double_bottom_volume_ratio = swing_config.get(
            "double_bottom_volume_ratio", 1.3
        )
    ...
    def multi_timeframe_momentum_system(self, df_5min, df_15min, df_60min):
        ...
        for i in range(10, len(df_60min_copy)):
            ...
            if len(bottom_indices) >= 2:
                ...
                # 检查第二个底是否不创新低
                if (
                    df_60min_copy.loc[right_bottom_idx, "low"]
                    >= df_60min_copy.loc[left_bottom_idx, "low"]
                ):
                    # 检查右底成交量是否大于左底成交量的1.3倍
                    if (
                        df_60min_copy.loc[right_bottom_idx, "volume"]
                        > df_60min_copy.loc[left_bottom_idx, "volume"]
                        * self.double_bottom_volume_ratio
                    ):
    ...
    def channel_mean_reversion_strategy(self, df):
        ...
        # 中轨向上穿越
        result_df["cross_middle_up"] = (result_df["close"] > result_df["bb_middle"]) & (
            result_df["close"].shift(1) <= result_df["bb_middle"].shift(1)
        )
        ...
        # 买入信号：之前N根K线内触碰下轨，现在向上穿过中轨 (向量化)
        touched_lower_recently = (
            result_df["touch_lower"]
            .rolling(window=self.bollinger_reversion_bars, min_periods=1)
            .apply(lambda x: x.any(), raw=True)
            .fillna(False)
            .shift(1)
        )
        result_df.loc[
            touched_lower_recently & result_df["cross_middle_up"], "channel_signal"
        ] = 1

        # 卖出信号：之前N根K线内触碰上轨，现在向下穿过中轨 (向量化)
        touched_upper_recently = (
            result_df["touch_upper"]
            .rolling(window=self.bollinger_reversion_bars, min_periods=1)
            .apply(lambda x: x.any(), raw=True)
            .fillna(False)
            .shift(1)
        ...
        result_df.loc[
            touched_upper_recently & result_df["cross_middle_down"], "channel_signal"
        ] = -1

        # 2. 过滤条件：结合日线级别的KDJ（9,3,3）金叉/死叉提高胜率 (修正逻辑)

        # 计算KDJ金叉/死叉
        result_df["kdj_golden_cross"] = (result_df["kdj_k"] > result_df["kdj_d"]) & (
            result_df["kdj_k"].shift(1) <= result_df["kdj_d"].shift(1)
        )

        result_df["kdj_death_cross"] = (result_df["kdj_k"] < result_df["kdj_d"]) & (
            result_df["kdj_k"].shift(1) >= result_df["kdj_d"].shift(1)
        )

        # 修正过滤逻辑：KDJ用于确认通道信号
        # 如果产生买入信号，但不是金叉状态(k > d)，则过滤
        result_df.loc[
            (result_df["channel_signal"] == 1)
            & (result_df["kdj_k"] <= result_df["kdj_d"]),
            "channel_signal",
        ] = 0
        # 如果产生卖出信号，但不是死叉状态(k < d)，则过滤
        result_df.loc[
            (result_df["channel_signal"] == -1)
            & (result_df["kdj_k"] >= result_df["kdj_d"]),
            "channel_signal",
        ...
        # 3. 风险控制：计算单次止损位为ATR（14日）的N倍
        # 注意：止损的实际执行逻辑需要在回测或实盘引擎中处理
        result_df["long_stop_loss"] = result_df["low"].shift(1) - (
            result_df["atr"] * self.atr_stop_loss_multiplier
        )
        result_df["short_stop_loss"] = result_df["high"].shift(1) + (
            result_df["atr"] * self.atr_stop_loss_multiplier
        )
        # # 若价格突破通道后一定时间未回归则强制止损的逻辑也需要在外部实现

        # 统计信号数量
        buy_signals = (result_df["channel_signal"] == 1).sum()
        sell_signals = (result_df["channel_signal"] == -1).sum()
        logger.info(
            f"通道回归均值策略信号计算完成，买入信号: {buy_signals}个, 卖出信号: {sell_signals}个"
        )
    ...
    def pattern_driven_reversal_strategy(self, df):
        ...
        for i in range(30, len(result_df)):
            ...
            for j in range(2, len(lows) - 2):
                if (
                    lows.iloc[j] == window["low"].iloc[j]
                    and lows.iloc[j] < lows.iloc[j - 2]
                    and lows.iloc[j] < lows.iloc[j + 2]
                ):
                    low_points.append(j)
            ...
            if len(low_points) >= 3:
                ...
                # 检查头部是否低于两肩
                if (
                    window["low"].iloc[head_idx] < window["low"].iloc[left_shoulder_idx]
                    and window["low"].iloc[head_idx]
                    < window["low"].iloc[right_shoulder_idx]
                ):

                    # 检查右肩成交量是否大于头部的50%
                    if (
                        window["volume"].iloc[right_shoulder_idx]
                        > window["volume"].iloc[head_idx]
                        * self.head_shoulder_volume_ratio
                    ):

                        # 计算颈线（左肩和右肩之间的高点）
                        neck_line = max(
                            window["high"].iloc[
                                left_shoulder_idx : right_shoulder_idx + 1
                            ]
                        )
                        ...
                        if window["close"].iloc[-1] > neck_line:
                            ...
                            if (
                                ...
                                > window["volume_ma_5"].iloc[-1]
                                * self.head_shoulder_breakout_volume_ratio
                            ):
                                result_df.loc[
                                    result_df.index[i], "head_shoulder_bottom"
                                ] = True
        ...
        for i in range(20, len(result_df)):
            ...
            for level in np.linspace(min(lows), max(lows), 20):
                touches = sum(
                    abs(low - level) < (max(lows) - min(lows)) * 0.03 for low in lows
                )
            ...
            if support_line is not None:
                ...
                for j in range(len(highs)):
                    if (
                        abs(highs[j] - max(highs[: j + 1]))
                        < (max(highs) - min(highs)) * 0.03
                    ):
                ...
                if len(resistance_touches) >= 2:
                    ...
                    support_touches_idx = [
                        ...
                        if abs(lows[j] - support_line) < (max(lows) - min(lows)) * 0.03
                    ]
                    ...
                    if len(support_touches_idx) >= 3:
                        ...
                        if (
                            window["volume"].iloc[third_touch_idx]
                            < window["volume_ma_5"].iloc[third_touch_idx]
                            * self.triangle_support_volume_ratio
                        ):
                            ...
                            if window["close"].iloc[-1] > max(highs[:-1]):
                                ...
                                if (
                                    ...
                                    result_df.loc[
                                        result_df.index[i], "ascending_triangle"
                                    ] = True
            ...
            for j in range(5, 10):
                if j < len(window):
                    ...
                    if pole_rise > self.flag_pole_min_rise:
                        ...
                        retracement = (pole_end - window["low"].iloc[j:].min()) / (
                            pole_end - pole_start
                        )
        ...
        for i in range(10, len(result_df)):
            # 查找价格新低但RSI未新低（看涨背离）
            if (
                result_df["low"].iloc[i] < result_df["low"].iloc[i - 5 : i].min()
                and result_df["rsi_14"].iloc[i]
                > result_df["rsi_14"].iloc[i - 5 : i].min()
            ):
                result_df.loc[result_df.index[i], "rsi_bullish_divergence"] = True

            # 查找价格新高但RSI未新高（看跌背离）
            if (
                result_df["high"].iloc[i] > result_df["high"].iloc[i - 5 : i].max()
                and result_df["rsi_14"].iloc[i]
                < result_df["rsi_14"].iloc[i - 5 : i].max()
            ):
                result_df.loc[result_df.index[i], "rsi_bearish_divergence"] = True

        # 综合形态驱动反转信号
        # 1 = 买入信号（头肩底或上升三角形或旗形整理，且有RSI看涨背离）
        # -1 = 卖出信号（RSI看跌背离 - 注意：仅基于背离，无看跌形态确认，可能需改进）
        # 0 = 无信号
    ...
    def hedge_position_balance(self, df, weekly_df=None):
        ...
        if weekly_df is not None and not weekly_df.empty:
            ...
            if all(
                ...
                # 计算TD序列 (简化版 Sell Setup & Countdown)
                # 注意: 这是一个简化的实现，未包含所有 TD Sequential 规则
                # 性能提示: 循环计算效率较低，可考虑使用专业库或更复杂的向量化
                weekly_df["td_setup_sell"] = 0
                td_setup_count = 0
                for i in range(4, len(weekly_df)):
                    if weekly_df["close"].iloc[i] > weekly_df["close"].iloc[i - 4]:
                        td_setup_count = (
                            td_setup_count + 1 if td_setup_count >= 0 else 1
                        )
                    else:
                        td_setup_count = 0
                    weekly_df.loc[weekly_df.index[i], "td_setup_sell"] = min(
                        td_setup_count, 9
                    )
                ...
                for i in range(len(weekly_df)):
                    if weekly_df["td_setup_sell"].iloc[i] >= 9:
                        in_countdown = True
                        td_countdown_count = 0

                    if in_countdown and i >= 2:
                        # Sell Countdown condition: Close higher than or equal to high 2 periods ago
                        if weekly_df["close"].iloc[i] >= weekly_df["high"].iloc[i - 2]:
                            td_countdown_count += 1
                        if (
                            td_countdown_count >= self.td_sequence_count
                        ):  # Check if count reaches target (e.g., 13)
                            weekly_df.loc[weekly_df.index[i], "td_countdown"] = (
                                self.td_sequence_count
                            )
                            in_countdown = False  # Reset after reaching target
                        else:
                            weekly_df.loc[weekly_df.index[i], "td_countdown"] = (
                                td_countdown_count
                            )
                        ...
                        weekly_df.loc[weekly_df.index[i], "td_countdown"] = (
                            td_countdown_count  # Carry over count if not in countdown or less than 2 bars
                        )
    ...
    def combine_signals(self, df):
        ...
        # hedge_balance 可能只返回 date, hedge_signal, hedge_ratio，需要从主 df 获取其他信号
        # 因此，这里的检查逻辑需要更灵活，或者确保上游传入包含所有信号的 df

        # 简化检查：假设df包含大部分信号，hedge_ratio 从 hedge_balance 合并进来
        base_signals = [
            "momentum_signal",
            "channel_signal",
            "pattern_signal",
        ...
        hedge_cols = ["hedge_signal", "hedge_ratio"]
        missing_base = [col for col in base_signals if col not in df.columns]
        missing_hedge = [col for col in hedge_cols if col not in df.columns]

        if missing_base:
            # 不再自动修复，而是记录错误并返回原始 df 或抛出异常
            logger.error(f"合并信号失败：缺少基本的信号列: {missing_base}")
            # raise ValueError(f"合并信号失败：缺少基本的信号列: {missing_base}") # 或者抛出异常
            return df  # 返回原始df

        # 如果缺少对冲列，可能意味着对冲策略未运行或未合并，使用默认值0
        if missing_hedge:
            logger.warning(f"缺少对冲信号列: {missing_hedge}，将使用默认值 0")
            for col in missing_hedge:
                df[col] = 0

        # # 移除自动修复逻辑
        # if missing_cols:
        #     logger.info(f"缺少必要的信号列: {missing_cols}，将自动生成这些信号")
        #     # ... (原自动生成逻辑已移除)
        #     missing_cols = [col for col in required_cols if col not in df.columns]
        #     if missing_cols:
        #         logger.error(f"生成信号后仍然缺少必要的列: {missing_cols}")
        #         return df
        ...
        # 计算加权综合信号
        df["weighted_signal"] = (
            df["momentum_signal"].fillna(0)
            * self.signal_weights.get("momentum_signal", 2.0)
            + df["channel_signal"].fillna(0)
            * self.signal_weights.get("channel_signal", 2.0)
            + df["pattern_signal"].fillna(0)
            * self.signal_weights.get("pattern_signal", 1.5)
            + df["hedge_signal"].fillna(0)
            * self.signal_weights.get(
                "hedge_signal", 1.0
            )  # 注意 hedge_signal 是 -1 或 0
            + df["volume_price_signal"].fillna(0)
            * self.signal_weights.get("volume_price_signal", 1.5)
        )
        ...
        # 买入信号：加权信号大于阈值
        df.loc[
            df["weighted_signal"] >= self.combined_signal_threshold, "combined_signal"
        ] = 1
        ...
        # 确保返回的结果中包含对冲仓位比例
        # 如果 hedge_ratio 不在 df 中，则添加默认值 0
        if "hedge_ratio" not in df.columns:
            df["hedge_ratio"] = 0
            logger.warning("在组合信号中未找到 hedge_ratio，使用默认值 0")

        # 统计信号数量
        buy_signals = (df["combined_signal"] == 1).sum()
        sell_signals = (df["combined_signal"] == -1).sum()
        hedge_positions = (
            df.loc[df["hedge_ratio"] > 0, "hedge_ratio"].mean()
            if (df["hedge_ratio"] > 0).any()
            else 0
        )

        logger.info(
            f"综合信号计算完成，买入信号: {buy_signals}个, 卖出信号: {sell_signals}个, "
            f"平均对冲仓位比例: {hedge_positions:.2%}"
        )
